/**
 * OpenAI WebSocket Voice Service with Ephemeral Tokens
 * This version uses server-generated ephemeral tokens for better security
 */
...
export class OpenAIWebSocketServiceWithTokens {
   ...
   */
  private async getEphemeralToken(): Promise<EphemeralTokenResponse> {
    const response = await fetch(`${this.config.serverUrl}/api/voice/ephemeral-token`, {
      method: 'POST',
      headers: {
  ...
  async disconnect(): Promise<void> {
    ...
    if (this.sessionId) {
      try {
        await fetch(`${this.config.serverUrl}/api/voice/session/${this.sessionId}`, {
          method: 'DELETE'
        });
      } catch (error) {
  ...
  private async checkTokenExpiry(): Promise<void> {
    if (this.tokenExpiry && Date.now() >= this.tokenExpiry) {
      this.log('Token expired, reconnecting...');
   ...
   */
  getSessionInfo(): { sessionId: string | null; tokenExpiry: number | null } {
    return {
  ...
  private log(message: string): void {
    console.log(`[OpenAIWebSocketService] ${message}`);
  }
...
export default OpenAIWebSocketServiceWithTokens;
